...
export async function PUT(
  ...
  try {
    ...
    const { data: updatedPaper, error } = await supabaseAdmin
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        entity_id: id,
        details: {
          message: `${status === 'submitted' ? 'Approved' : status === 'rejected' ? 'Rejected' : 'Updated'} paper: ${updatedPaper.title}`,
          cmt_paper_id: updatedPaper.cmt_paper_id,
          status,
